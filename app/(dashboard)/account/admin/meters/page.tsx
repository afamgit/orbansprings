...
import {MetersChart} from '@/app/components/meters-chart'


export default async function Page() {

const allMeters = await prisma.meters.findMany()
...
const volumePurchased = async (mid: string, mtype: string) => {
  ...
  return totalDelivered. _sum.md_volume_received || '-'
      ...
      return (
        <main className='w-full flex flex-col justify-start items-center'>
          ...
          <h2 className='font-bold text-2xl'>Meters</h2>
         <Link className='rounded-full px-3 py-2 bg-gray-800 text-white' href='/account/admin/meters/generate-numbers'>Generate meter numbers</Link>
         </div>
  ...
  <thead>
    <tr className='bg-gray-300 px-2 py-1'>
      ...
      <th className='px-2 py-1'>Meter Info</th>
      <th className='px-2 py-1'>User type</th>
      <th className='px-2 py-1'>Total vol sold(gl)</th>
      <th className='px-2 py-1'>Volume Purchased(gl)</th>
